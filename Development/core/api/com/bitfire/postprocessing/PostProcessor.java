...
public final class PostProcessor implements Disposable {
	...
	/** Construct a new PostProcessManager with FBO dimensions set to the size of the screen */
	...
	/** Construct a new PostProcessManager with the given parameters, defaulting to <em>TextureWrap.ClampToEdge</em> as texture wrap mode */
	...
	/** Construct a new PostProcessManager with the given parameters and viewport, defaulting to <em>TextureWrap.ClampToEdge</em> as
	 * texture wrap mode */
	...
	/** Construct a new PostProcessManager with the given parameters, viewport and the specified texture wrap mode */
	...
	/** Construct a new PostProcessManager with the given parameters and the specified texture wrap mode */
	public PostProcessor (int fboWidth, int fboHeight, boolean useDepth, boolean useAlphaChannel, boolean use32Bits,
	...
	/** Sets the listener that will receive events triggered by the PostProcessManager rendering pipeline. */
	public void setListener (PostProcessorListener listener) {
		this.listener = listener;
	}

	/** Adds the specified effect to the effect chain and transfer ownership to the PostProcessManager, it will manage cleaning it up for
	...
	public boolean capture () {
		...
		if (enabled && !capturing) {
			if (buildEnabledEffectsList() == 0) {
				// no enabled effects
				// Gdx.app.log( "PostProcessManager::capture()",
	...
	public boolean captureNoClear () {
		...
		if (enabled && !capturing) {
			if (buildEnabledEffectsList() == 0) {
				// no enabled effects
				// Gdx.app.log( "PostProcessManager::captureNoClear",
				// "No post-processor effects enabled" );
				return false;
			}
	...
	public void render (FrameBuffer dest) {
		...
		if (count > 0) {
			...
			Gdx.app.log("PostProcessManager", "No post-processor effects enabled, aborting render");
		}
	}
